...
- (void)updateThreadUserBar:(DZUIUserModel *)UIModel style:(DZDUserStyle *)UserStyle{
    ...
    NSString *followStr = (UIModel.user_isFollow == 1) ? @"已关注" : ((UIModel.user_isFollow == 2) ? @"相互关注" : @"加关注 +");
    [self.followButton setTitle:followStr forState:UIControlStateNormal];
...
-(UIButton *)followButton{
    if (!_followButton) {
        _followButton = [UIButton ButtonNormalWithFrame:CGRectZero title:@"关注 +" titleFont:KFont(16) titleColor:KTitle_Color normalImgPath:@"" touchImgPath:@"" isBackImage:NO picAlpha:1];
