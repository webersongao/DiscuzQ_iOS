...
- (void)setAttachmentBar {
    ...
    self.attachmentBar.layer.borderColor = KLine_Color.CGColor;
...
- (void)keyboardShow:(NSNotification *)note {
    ...
    if (![self dz_intersectsWithAnotherView:nil]) {
        return;
    }
...
-(void)resignTextView:(NSInteger)tag {
    ...
    if (!self.imageboaudIsShow) { // 使自定义表情出现
        ...
        if (tag == 101) {
            [self.textBarView.faceBtn setBackgroundImage:[UIImage imageNamed:@"dz_keyboardbtn_back"] forState:UIControlStateNormal];
        }
