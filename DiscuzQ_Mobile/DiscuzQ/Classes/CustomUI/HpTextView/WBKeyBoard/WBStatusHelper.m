...
+ (NSBundle *)bundle {
    ...
    dispatch_once(&onceToken, ^{
        NSString *path = [[NSBundle mainBundle] pathForResource:@"dzxResource" ofType:@"bundle"];
...
+ (NSBundle *)emoticonBundle {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *bundlePath = [[NSBundle mainBundle] pathForResource:@"dzxEmoticon" ofType:@"bundle"];
...
+ (NSDictionary *)emoticonDic {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        NSString *emoticonBundlePath = [[NSBundle mainBundle] pathForResource:@"dzxEmoticon" ofType:@"bundle"];
...
+ (NSMutableDictionary *)_emoticonDicFromPath:(NSString *)path {
    ...
    if (json.length) {
        group = [WBEmoticonGroup DZQ_ModelWithJSON:json];
    }
    if (!group) {
        NSString *plistPath = [path stringByAppendingPathComponent:@"Eminfo.plist"];
        NSDictionary *plist = [NSDictionary dictionaryWithContentsOfFile:plistPath];
        if (plist.count) {
            group = [WBEmoticonGroup DZQ_ModelWithJSON:plist];
        }
    }
...
+ (NSArray<WBEmoticonGroup *> *)emoticonGroups {
    ...
    dispatch_once(&onceToken, ^{
        ...
        groups = (NSMutableArray *)[NSArray DZQ_ModelArrayWithClass:[WBEmoticonGroup class] json:packages];
        ...
        for (int i = 0, max = (int)groups.count; i < max; i++) {
            ...
            [group DZQ_ModelSetWithDictionary:info];
