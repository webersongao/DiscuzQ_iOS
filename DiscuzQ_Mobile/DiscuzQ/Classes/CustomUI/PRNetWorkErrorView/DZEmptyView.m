...
//每个子控件之间的间距
#define kEM_SubViewMargin 20.f

//描述字体
#define kEM_TitleLabFont [UIFont systemFontOfSize:16.f]

//详细描述字体
#define kEM_DetailLabFont [UIFont systemFontOfSize:14.f]

//按钮字体大小
#define kEM_ActionBtnFont  [UIFont systemFontOfSize:14.f]

//按钮高度
#define kEM_ActionBtnHeight 40.f
//按钮宽度
#define kEM_ActionBtnWidth 120.f
//水平方向内边距
#define kEM_ActionBtnHorizontalMargin 30.f

//背景色
#define kEM_BackgroundColor [UIColor colorWithRed:250.f/255.f green:250.f/255.f blue:250.f/255.f alpha:1.f]
//黑色
#define kEM_BlackColor [UIColor colorWithRed:0.3f green:0.3f blue:0.3f alpha:1.f]
//灰色
#define kEM_GrayColor [UIColor colorWithRed:0.5f green:0.5f blue:0.5f alpha:1.f]

@interface DZEmptyView ()
...
- (void)setupSubviews{
    ...
    contentMaxWidth = self.emptyViewIsCompleteCoverSuperView ? self.dem_width : self.dem_width - 30.f;
    contentWidth = 0;//内容物宽度
    contentHeight = 0;//内容物高度
    subViweMargin = self.subViewMargin ? self.subViewMargin : kEM_SubViewMargin;
    ...
    //自定义view
    if (self.customView) {
        contentWidth = self.customView.dem_width;
        contentHeight = self.customView.dem_maxY;
    }
...
- (void)setSubViewFrame{
    ...
    //emptyView初始宽高
    CGFloat originEmptyWidth = self.dem_width;
    CGFloat originEmptyHeight = self.dem_height;
    ...
    if (!self.emptyViewIsCompleteCoverSuperView) {
        self.dem_size = CGSizeMake(contentWidth, contentHeight);
    }
    self.center = CGPointMake(emptyViewCenterX, emptyViewCenterY);
    
    //设置contentView
    self.contentView.dem_size = CGSizeMake(contentWidth, contentHeight);
    if (self.emptyViewIsCompleteCoverSuperView) {
    ...
    //子控件的centerX设置
    CGFloat centerX = self.contentView.dem_width * 0.5f;
    if (self.customView) {
        self.customView.dem_centerX      = centerX;
        
    }else{
        _promptImageView.dem_centerX = centerX;
        _titleLabel.dem_centerX       = centerX;
        _detailLabel.dem_centerX = centerX;
        _actionButton.dem_centerX    = centerX;
    }
    
    if (self.contentViewOffset) { //有无设置偏移
        self.dem_centerY += self.contentViewOffset;
        
    } else if (self.contentViewY < 1000) { //有无设置Y坐标值
        self.dem_y = self.contentViewY;
    ...
    if (self.ignoreContentInset && [self.superview isKindOfClass:[UIScrollView class]]) {
        ...
        self.dem_centerY -= scrollView.contentInset.top;
        self.dem_centerX -= scrollView.contentInset.left;
        self.dem_centerY += scrollView.contentInset.bottom;
        self.dem_centerX += scrollView.contentInset.right;
    }
}
...
- (void)setupPromptImageView:(UIImage *)img{
    ...
    contentWidth = self.promptImageView.dem_size.width;
    contentHeight = self.promptImageView.dem_maxY;
}

- (void)setupTitleLabel:(NSString *)titleStr{
    
    UIFont *font = self.titleLabFont.pointSize ? self.titleLabFont : kEM_TitleLabFont;
    CGFloat fontSize = font.pointSize;
    UIColor *textColor = self.titleLabTextColor ? self.titleLabTextColor : kEM_BlackColor;
    ...
    contentHeight = self.titleLabel.dem_maxY;
}

- (void)setupDetailLabel:(NSString *)detailStr{
    
    UIColor *textColor = self.detailLabTextColor ? self.detailLabTextColor : kEM_GrayColor;
    UIFont *font = self.detailLabFont.pointSize ? self.detailLabFont : kEM_DetailLabFont;
    CGFloat fontSize = font.pointSize;
    ...
    contentWidth = width > contentWidth ? width : contentWidth;
    contentHeight = self.detailLabel.dem_maxY;
   
}

- (void)setupActionBtn:(NSString *)btnTitle target:(id)target action:(SEL)action btnClickBlock:(DZActionTapBlock)btnClickBlock{
    
    UIFont *font = self.actionBtnFont.pointSize ? self.actionBtnFont : kEM_ActionBtnFont;
    CGFloat fontSize = font.pointSize;
    UIColor *titleColor = self.actionBtnTitleColor ?: kEM_BlackColor;
    ...
    contentHeight = self.actionButton.dem_maxY;
}
...
- (void)setEmptyViewIsCompleteCoverSuperView:(BOOL)emptyViewIsCompleteCoverSuperView{
    ...
    if (emptyViewIsCompleteCoverSuperView) {
        if (!self.backgroundColor || [self.backgroundColor isEqual:[UIColor clearColor]]) {
            self.backgroundColor = kEM_BackgroundColor;
        }
        [self setNeedsLayout];
    }else{
