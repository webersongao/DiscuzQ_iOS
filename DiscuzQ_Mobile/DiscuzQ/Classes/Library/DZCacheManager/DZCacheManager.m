...
NSString *const PathSpace =@"com.516680.DZNetKit";
...
@interface DZCacheManager ()
@property (nonatomic ,copy)NSString *diskCachePath;
@property (nonatomic ,strong) dispatch_queue_t operationQueue;

@end

@implementation DZCacheManager

+ (DZCacheManager *)sharedInstance{
    static DZCacheManager *cacheInstance=nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        cacheInstance = [[DZCacheManager alloc] init];
    });
...
- (id)init{
    ...
    if (self) {
        ...
        _operationQueue = dispatch_queue_create("com.dispatch.DZCacheManager", DISPATCH_QUEUE_SERIAL);
...
- (void)initCachesfileWithName:(NSString *)name{
    ...
    self.diskCachePath =[[self DZQKitPath] stringByAppendingPathComponent:name];
