...
- (NSString *)startRecord {
    ...
    if (session == nil) {
        KSLog(@"Error create session:%@", [sessionError description]);
    } else {
   ...
   NSURL *recordFileUrl = [NSURL fileURLWithPath:self.recordPath];
    ...
    if (_recorder) {
        ...
        KSLog(@"音频格式和文件存储格式不匹配,无法初始化Recorder");
...
- (void)transformMp3:(NSString *)recordPath {
    ...
    NSString *nowTime = [NSString stringWithFormat:@"luyin%lu",(unsigned long)self.audioArray.count];
...
- (void)playlistAudio:(NSURL *)url {
    ...
    KSLog(@"%lu",self.player.data.length/1024);
