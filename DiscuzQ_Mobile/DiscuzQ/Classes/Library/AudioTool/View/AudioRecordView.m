...
//  testYUyin
//
//  Created by WebersonGao on 2017/5/11.
...
- (void)sliderValueChanged:(UISlider *)slider {
    KSLog(@"slider value%f",slider.value);
...
- (void)recordingAction:(NSTimer *)timer {
    ...
    self.timeLab.text = [NSString stringWithFormat:@"%ld秒",(long)self.recordTime];
...
- (void)stopRecord {
    self.tipLab.text = @"播放";
    self.tipLab.textColor = KGray_Color;
...
- (void)playRecord {
    ...
    self.timeLab.text = [NSString stringWithFormat:@"%ld秒",(long)self.playTime];
