...
@property(nonatomic,strong) DZScaleView *shadowScaleView;
...
-(void)updateAlertViewSize:(CGSize)viewSize{
    ...
    self.shadowScaleView.size = viewSize;
    self.shadowScaleView.center = CGPointMake(self.backContainerView.width/2.f, self.backContainerView.height/2.f);
}
...
-(void)dismissScaleAlertView{
    ...
    [UIView animateWithDuration:0.25 animations:^{
        ...
        self.shadowScaleView.transform = CGAffineTransformMakeScale(0.1, 0.1);
    } completion:^(BOOL finished) {
        //移除View
        if (self.shadowScaleView.dismissBlock) {
            self.shadowScaleView.dismissBlock();
        }
        [self.shadowScaleView removeFromSuperview];
