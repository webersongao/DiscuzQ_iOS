...
@interface DZHomeCateController ()
{
    CGRect m_frame;
}
...
@implementation DZHomeCateController
...
- (void)downLoadCateListData:(NSInteger)page {
    ...
    KWEAKSELF
    [[DZNetCenter center] dzx_threadListWithCate:self.cateModel.pid page:page completion:^(NSArray<DZQDataThread *> *varModel, BOOL hasMore,BOOL success) {
        [weakSelf.HUD hide];
        if (success) {
