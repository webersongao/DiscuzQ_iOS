...
@implementation DZNotiDetailStyle

+(DZNotiDetailStyle *)inner_dataStyle:(DZQDataNoti *)dataModel{
    
    DZNotiDetailStyle *localStyle = [DZNotiDetailStyle DNotiDetailStyleWithCellWidth:KScreenWidth dataModel:dataModel];
...
+(instancetype)DNotiDetailStyleWithCellWidth:(CGFloat)cellWidth dataModel:(DZQDataNoti *)dataModel{
    ...
    // 开始计算布局
    DZNotiDetailStyle *localStyle = [[DZNotiDetailStyle alloc] init];
...
+(NSString *)contentWithType:(DZQNotiV1 *)model{
    ...
    if ([model.type isEqualToString:@"system"]) {
        ...
        // @我的
        contentStr = model.post_content;
    }else if ([model.type isEqualToString:@"questioned"]){
        // 问答通知
        contentStr = model.post_content;
