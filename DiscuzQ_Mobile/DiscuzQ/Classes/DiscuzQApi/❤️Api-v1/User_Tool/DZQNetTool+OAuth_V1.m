...
#import "DZQNetTool+OAuth_V1.h"
#import "DZQNetTool_Private.h"

@implementation DZQNetTool (OAuth_V1)
...
-(void)DZQ_V1_RegisterWithName:(NSString *)username password:(NSString *)password token:(NSString *)token mobile:(NSString *)mobile code:(NSString *)code success:(PRCompleteBlock)success failure:(PRFailureBlock)failure{
    ...
    NSString *UrlString = DZQUrlCommonPara(DZQ_V1_User_register,nil,nil);
    ...
    [self.NetClient baidu_PostReqWithUrl:UrlString urlTag:0 parameters:paraDict success:^(id response) {
        [weakQSelf.dataLogic resModelWithJSON:response urlCtrl:KDZQMapKey(DZQ_V1_User_register, @"register") completion:^(DZQBaseRes *resModel) {
            if (success) {
...
-(void)DZQ_V1_LoginWithName:(NSString *)username password:(NSString *)password token:(NSString *)token mobile:(NSString *)mobile success:(PRCompleteBlock)success failure:(PRFailureBlock)failure{
    ...
    NSString *UrlString = DZQUrlCommonPara(DZQ_V1_User_login,nil,nil);
    ...
    DZQWEAKSELF
    [self.NetClient baidu_PostReqWithUrl:UrlString urlTag:0 parameters:paraDict success:^(id response) {
        [weakQSelf.dataLogic resModelWithJSON:response urlCtrl:KDZQMapKey(DZQ_V1_User_login, @"login") completion:^(DZQBaseRes *resModel) {
...
-(void)DZQ_V1_RefreshUserToken:(NSString *)refresh_token scope:(NSArray *)scope client_id:(NSString *)client_id client_secret:(NSString *)client_secret success:(PRCompleteBlock)success failure:(PRFailureBlock)failure{
    ...
    NSString *UrlString = DZQUrlCommonPara(DZQ_V1_User_refreshToken,nil,nil);
    ...
    DZQWEAKSELF
    [self.NetClient baidu_PostReqWithUrl:UrlString urlTag:0 parameters:paraDict success:^(id response) {
        [weakQSelf.dataLogic resModelWithJSON:response urlCtrl:KDZQMapKey(DZQ_V1_User_refreshToken, @"refreshToken") completion:^(DZQBaseRes *resModel) {
            if (success) {
                success(resModel,resModel.success);
            }
        }];
