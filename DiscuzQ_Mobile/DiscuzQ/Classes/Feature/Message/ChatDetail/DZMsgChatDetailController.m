...
@property (nonatomic, strong) NSMutableArray<oldMssageModel *> *messageModelArr;
...
- (void)goOtherCenter:(UITapGestureRecognizer *)sender {
    ...
    oldMssageModel *model = self.messageModelArr[view.tag];
...
- (void)deleteMessage {
    ...
    oldMssageModel *msgModel = self.messageModelArr[self.pressIndexRow];
...
- (void)copyText {
    oldMssageModel *message = self.messageModelArr[self.pressIndexRow];
...
- (NSMutableArray<oldMssageModel *> *)messageModelArr {
    if (!_messageModelArr) {
