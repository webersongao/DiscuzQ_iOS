...
//手势密码
#import "DZGestureLockerView.h"


@implementation DZLockerView{
    UIButton *button[9];
}
...
@implementation DZGestureLockerView
...
-(instancetype)initWithFrame:(CGRect)frame{
    if(self = [super initWithFrame:frame]){
        ...
        UIView *line = [[UIView alloc]initWithFrame:makeRect(0, 0, kScreenWidth, 1)];
        ...
        for(int i=0;i<Number;++i){
            for(int j=0;j<Number;++j){
                ...
                if(j==0)btn.left=30;
                else if (j==1)btn.centerX = kScreenWidth*0.5;
                else btn.right = kScreenWidth-30;
                
                if(i==0)btn.top=20;
                else if (i==1)btn.centerY = 20+kScreenWidth*0.5-30;
                else btn.bottom = 20+kScreenWidth-60;
...
-(void)notiDelegate{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(DZGestureLockerViewDidSetFinished:)]){
        [_delegate DZGestureLockerViewDidSetFinished:arr];
    }
}
