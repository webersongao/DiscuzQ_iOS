...
@implementation DZQVideoImageLayer
...
-(void)buttonPressed:(UIButton *)sender{
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(DZQVideoImageLayerButtonClick:item:)]){
        [_delegate DZQVideoImageLayerButtonClick:sender item:_item];
    }
...
-(void)sliderValueChanged:(UISlider *)slider{
    KSLog(@"当前值：%f",slider.value);
    if(_delegate && [_delegate respondsToSelector:@selector(DZQVideoImageLayerSliderEventValueChanged:item:)]){
        [_delegate DZQVideoImageLayerSliderEventValueChanged:slider item:_item];
    }
}


-(void)setStatus:(DZQVideoLayerStatus)status{
    _status = status;
    
    switch (_status) {
            //未播放静止（播放按钮）
        case DZQVideoLayerValue1:{
            
        }
            break;
            //未播放静止+点击了空白处（所有控件）
        case DZQVideoLayerValue2:{
            
        }
            break;
            //播放中
        case DZQVideoLayerValue3:{
            _playCenterButton.selected = YES;
            _playLeftButton.selected = YES;
        }
            break;
            //播放中+点击了空白处（所有控件）
        case DZQVideoLayerValue4:{
            
        }
            break;
            //播放暂停中（所有控件）
        case DZQVideoLayerValue5:{
            
        }
            break;
            //播放暂停+点击空白处
        case DZQVideoLayerValue6:{
            
        }
            break;
            //播放完毕 （播放按钮）
        case DZQVideoLayerValue7:{
            _playCenterButton.selected = NO;
            _playLeftButton.selected = NO;
        }
            break;
            //播放完毕+点击空白处（所有控件）
        case DZQVideoLayerValue8:{
...
//视频视图
@implementation DZQMediaVideoCell
...
-(instancetype)initWithItem:(DZQMediaGroupItem *)item{
    if(self = [super init]){
        ...
        _mVideoImagelayer = [[DZQVideoImageLayer alloc]initWithItem:_item];
...
-(void)setShowType:(DZQImageShowType)showType{
    _showType = showType;
    
    switch (_showType) {
            //未展示
        case DZQImageShowValue1:{
            
        }
            break;
            //放大展示
        case DZQImageShowValue2:{
            [_player play];
            [self addPeriodicTime];
            self.mVideoImagelayer.status = DZQVideoLayerValue3;
        }
            break;
            //滚动展示
        case DZQImageShowValue3:{
            
        }
            break;
            //滚动隐藏
        case DZQImageShowValue4:{
...
-(void)addPeriodicTime{
    ...
    __block DZQMediaVideoCell *weakSelf = self;
...
-(void)moviePlayDidEnd{
    ...
    [_player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
        [self.player pause];
        self.mVideoImagelayer.status = DZQVideoLayerValue7;
    }];
}
...
-(void)DZQVideoImageLayerButtonClick:(UIButton *)sender item:(DZQMediaGroupItem *)item{
    ...
    if(sender.tag==100){
        [_player seekToTime:CMTimeMake(0, 1) completionHandler:^(BOOL finished) {
            [self.player pause];
            self.mVideoImagelayer.status = DZQVideoLayerValue7;
            if(self.videoViewDelegate && [self.videoViewDelegate respondsToSelector:@selector(DZQMediaVideoCellImageButtonClick:item:)]){
                [self.videoViewDelegate DZQMediaVideoCellImageButtonClick:sender item:item];
            }
        }];
    }
    else{
