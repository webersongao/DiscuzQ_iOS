...
@implementation DZQChatEmotionModel

+ (DZQChatEmotionModel *)emotionWithDict:(NSDictionary*)dic atIndex:(int)index{
    
    DZQChatEmotionModel* model = [DZQChatEmotionModel new];
...
static DZQChatEmotionCenter *emotion = nil;

@implementation DZQChatEmotionCenter

+ (DZQChatEmotionCenter *)sharedCenter{
    static dispatch_once_t predicate;
    dispatch_once(&predicate, ^{
        emotion = [DZQChatEmotionCenter new];
    });
    return emotion;
}
...
- (void)initEmotionImages{
    ...
    DZQChatEmotionModel* model = nil;
    NSDictionary* dic = nil;
    for (int i = 0; i < array.count; i++) {
        dic = array[i];
        model = [DZQChatEmotionModel emotionWithDict:dic atIndex:i];
...
-(NSMutableArray *)dealWithArray:(NSMutableArray *)arr1 arr2:(NSMutableArray *)arr2{
    ...
    NSInteger k = 1;
    NSInteger number = DZQChatEmojiRow * DZQChatEmojiLine;
...
//表单Layout
@implementation DZQChatEmotionLayout

- (void)prepareLayout {
    [super prepareLayout];
    
    CGFloat W = (self.collectionView.bounds.size.width) / DZQChatEmojiLine;
    CGFloat H = (self.collectionView.bounds.size.height) / DZQChatEmojiRow;
    ...
    CGFloat Y = (self.collectionView.bounds.size.height - DZQChatEmojiRow * H);
