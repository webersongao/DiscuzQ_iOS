...
+ (instancetype)sharedView
{
    ...
    static DZQRecordHUD *sharedView;
    dispatch_once(&once, ^ {
        sharedView = [[DZQRecordHUD alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
+ (void)show {
    [[DZQRecordHUD sharedView] show];
}

+ (void)changeSubTitle:(NSString *)str
{
	[DZQRecordHUD sharedView].subTitleLabel.text = str;
}

+ (void)dismissWithSuccess:(NSString *)str
{
	[[DZQRecordHUD sharedView] dismiss:str];
}
...
- (void)show
{
	...
	self.titleLabel.center = CGPointMake(kScreenWidth/2,kScreenHeight/2 - 30);
	self.titleLabel.text = @"倒计时";
	
	self.subTitleLabel.center = CGPointMake(kScreenWidth/2,kScreenHeight/2 + 30);
	self.subTitleLabel.text = @"向上滑动取消";
	
	self.centerLabel.center = CGPointMake(kScreenWidth/2,kScreenHeight/2);
