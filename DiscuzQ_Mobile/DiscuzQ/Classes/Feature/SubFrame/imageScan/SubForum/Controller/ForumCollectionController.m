...
- (void)configCollectionController {
    ...
    self.collectionView.mj_header = [DZRefreshHeader headerWithRefreshingBlock:^{
        [weakSelf loadForumIndexDataWitLoadType:DZRLoadRefresh];
    }];
}

- (void)loadCacheRequest {
    [self.HUD showLoadingMessag:@"正在刷新" toView:self.view];
    [self loadForumIndexDataWitLoadType:DZRLoadCache]; // 读缓存，没有缓存的话自己会请求网络
    if ([DZRequestManager isCache:self.urlString andParameters:nil]) { // 缓存有的话，要刷新一次。缓存没有的话，不请求了，上面那个方法已经请求了
        [self loadForumIndexDataWitLoadType:DZRLoadRefresh];
    }
}

- (void)loadForumIndexDataWitLoadType:(DZRLoadType)type {
    
    [DZThreadNetTool DZ_DownloadForumCategoryData:type isCache:YES completion:^(DiscoverModel *indexModel) {
        [self.HUD hide];
        if (indexModel) {
