...
- (void)config_CateHeader {
    ...
    self.timeLabel.frame = CGRectMake(self.width - 80 - kMargin15, (self.height/2.f)-6, 80, 12.f);
...
-(UILabel *)titleLabel{
    if (!_titleLabel) {
        _titleLabel = [UILabel labelWithFrame:CGRectZero title:@"未分类" titleColor:K_Color_Theme fontSize:17.f textAlignment:NSTextAlignmentLeft];
    }
    return _titleLabel;
}
...
-(UILabel *)timeLabel{
    if (!_timeLabel) {
        _timeLabel = [UILabel labelWithFrame:CGRectZero title:@"排名：暂无" titleColor:KGray_Color fontSize:12.f textAlignment:NSTextAlignmentRight];
    }
    return _timeLabel;
}
...
- (UILabel *)descLabel {
    if (!_descLabel) {
        _descLabel = [UILabel labelWithFrame:CGRectZero title:@"主题分类描述文字" titleColor:KBlack_Color fontSize:14.f textAlignment:NSTextAlignmentLeft];
    }
    return _descLabel;
}
